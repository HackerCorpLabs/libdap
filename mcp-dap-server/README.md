...
### C source-level debugging session

Debug a C program with local variable inspection and C-level stack traces. Requires the program to be compiled with `cc -g` and a `.srcmap` file from `nd100-ld -m`.
...
3. debug_set_breakpoints(source="hello.c", lines=[12])   -- break at sum=0
4. debug_continue()                                       -- stops at hello.c:12
5. debug_variables(scope="Locals")                        -- shows C locals:
   -> n=10 (param), sum=0 (local), i=1 (local)
6. debug_step_over()                                      -- steps to next C line
7. debug_stack_trace()                                    -- C-level call stack:
   -> sum_to_n (hello.c:12) <- main (hello.c:22)
8. debug_variables(scope="Registers")                     -- CPU registers including
                                                             B (frame pointer)
9. debug_disconnect()
```

The `.srcmap` file supports mixed C and assembly programs. Source lines are attributed to their respective `.c` or `.s` files based on STABS metadata, so stepping through a C function that calls an assembly routine will show the correct source file for each frame.
...
- **Map files**: Pass the `.srcmap` file generated by `nd100-ld -m`. For assembly-only programs, this contains line-to-address mappings (`source.s:LINE -> OCTAL_ADDRESS`). For C programs compiled with `cc -g`, it also includes function definitions, parameter offsets, and local variable offsets (FUNC/PARAM/LOCAL entries).
- **C locals**: When stopped inside a C function (detected via FUNC entries in the srcmap), the `"Locals"` scope shows C parameters and local variables read from the B-register frame. Parameters are at B+2, B+3, etc.; locals at B-1, B-2, etc.
