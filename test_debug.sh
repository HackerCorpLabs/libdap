#!/bin/bash
# Start nd100x emulator and launch dap_debugger for manual testing
# of the C hello program with full source-level debugging.
#
# Usage: ./test_debug.sh
#        ./test_debug.sh --no-client   # start only nd100x, no debugger client

set -e

ND100X=~/repos/nd100x/build/bin/nd100x
DAP_DEBUGGER=~/repos/libdap/build/bin/dap_debugger
...
if [ ! -x "$DAP_DEBUGGER" ]; then
    echo "ERROR: dap_debugger not found at $DAP_DEBUGGER"
...
if [ "$1" = "--no-client" ]; then
    ...
    echo "  $DAP_DEBUGGER $SOURCE --stop-on-entry"
    echo ""
    ...
    echo ""
    echo "Launching dap_debugger..."
    echo ""
    $DAP_DEBUGGER "$SOURCE" --stop-on-entry --debug || true
fi
